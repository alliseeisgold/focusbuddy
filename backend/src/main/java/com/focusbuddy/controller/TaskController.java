...
public class TaskController {
    ...
    @PostMapping("/current")
    public ResponseEntity<Task> createTodayTask(@AuthenticationPrincipal User currentUser,
    ...
    @PostMapping("/planned")
    public ResponseEntity<Task> createPlannedTask(@AuthenticationPrincipal User currentUser,
                                                  @RequestBody TaskDto taskDto) {
    ...
    public ResponseEntity<?> updateTask(@AuthenticationPrincipal User currentUser,
        ...
        Task existingTask = taskService.getTaskByIdAndUser(id, currentUser)
        ...
        if (taskDto.getDueDate() != null) {
            if (existingTask.getIsCurrent()) {
                ...
                if (taskDto.getDueDate().isAfter(LocalDate.now())) {
                    ...
                    return ResponseEntity.badRequest().body("For planned tasks the due date must be in the future");
                }
            }
        }
    ...
    public ResponseEntity<?> deleteTask(@AuthenticationPrincipal User currentUser, @PathVariable Long id) {
        if (currentUser == null || userRepository.findByUsername(currentUser.getUsername()).isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("User not found");
        }
