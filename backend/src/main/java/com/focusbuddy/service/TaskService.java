...
public class TaskService {
  ...
  public List<Task> getCurrentTasksForUser(User user) {
    return getAllTasksForUser(user).stream().filter(Task::getIsCurrent).toList();
  }

  public List<Task> getPlannedTasksForUser(User user) {
    return getAllTasksForUser(user).stream().filter(task -> !task.getIsCurrent()).toList();
  }
  ...
  public List<Task> getCompletedTasksForUser(User user) {
    LocalDate tomorrow = LocalDate.now().plusDays(1);
    return getPlannedTasksForUser(user).stream().filter(Task::getIsCompleted).toList();
  }
